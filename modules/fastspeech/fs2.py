...
from modules.commons.conformer.conformer import ConformerDecoder, ConformerEncoder
...
# from modules.commons.former.former import EBranchformerEncoder, ConformerEncoder, BranchformerEncoder, ConformerDecoder, BranchformerDecoder, EBranchformerDecoder
...
FS_ENCODERS = {
    'fft': lambda hp, embed_tokens, d: FastspeechEncoder(
        embed_tokens, hp['hidden_size'], hp['enc_layers'], hp['enc_ffn_kernel_size'],
        num_heads=hp['num_heads']),
    'conformer': lambda hp, embed_tokens, d: ConformerEncoder(
        embed_tokens, hp['hidden_size'], hp['enc_layers'], hp['enc_ffn_kernel_size'],
        num_heads=hp['num_heads']),

    'branchformer': lambda hp, embed_tokens, d: BranchformerEncoder(
        embed_tokens, hp['hidden_size'], hp['enc_layers'], hp['enc_ffn_kernel_size'],
        num_heads=hp['num_heads']),
    'ebranchformer': lambda hp, embed_tokens, d: EBranchformerEncoder(
        embed_tokens, hp['hidden_size'], hp['enc_layers'], hp['enc_ffn_kernel_size'],
        num_heads=hp['num_heads']),
}
...
FS_DECODERS = {
    ...
    'conformer': lambda hp: ConformerDecoder(
        hp['hidden_size'], hp['dec_layers'], hp['dec_ffn_kernel_size'], hp['num_heads']),
    'branchformer': lambda hp: BranchformerDecoder(
        hp['hidden_size'], hp['dec_layers'], hp['dec_ffn_kernel_size'], hp['num_heads']),
    'ebranchformer': lambda hp: EBranchformerDecoder(
        hp['hidden_size'], hp['dec_layers'], hp['dec_ffn_kernel_size'], hp['num_heads']),
}
...
class FastSpeech2(nn.Module):
    def __init__(self, dictionary, out_dims=None):
        ...
        #self.encoder = FS_ENCODERS[hparams['encoder_type']](hparams, len(self.dictionary), self.dictionary)
