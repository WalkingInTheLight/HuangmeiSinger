...
class Attention(nn.Module):
    def __init__(self, input_dim):
        ...
        self.attention = nn.Linear(input_dim, 1, bias=False, device='cuda')

    def forward(self, encoder_outputs):
        attention_weights = torch.softmax(self.attention(encoder_outputs), dim=1)

        weighted_output = torch.sum(attention_weights * encoder_outputs, dim=1)
...
class FastSpeech2MIDI(FastSpeech2):
    def __init__(self, dictionary, out_dims=None):
        ...
        # self.encoder2 = FS_ENCODERS['branchformer'](hparams, self.encoder_embed_tokens, self.dictionary)
