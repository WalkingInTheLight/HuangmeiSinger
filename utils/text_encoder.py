...
if six.PY2:  # python2结果为True
    RESERVED_TOKENS_BYTES = RESERVED_TOKENS
else:
    RESERVED_TOKENS_BYTES = [bytes(PAD, "ascii"), bytes(EOS, "ascii")]  # 列表[b'<PAD>',b'<EOS>']
...
class TextEncoder(object):
    ...
    def encode(self, s):
        ...
        # ID 应在 [num_reserved_ids、vocab_size） 范围内。ID [0，num_reserved_ids）保留。

        EOS is not appended.  # 不附加 EOS。
...
class TokenTextEncoder(TextEncoder):
    ...
    def sil_phonemes(self):
        #print("OKKKKKKKKKKKKKK: ", self._id_to_token.values())
