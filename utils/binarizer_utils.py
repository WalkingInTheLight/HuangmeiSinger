...
def get_pitch(wav_data, mel, hparams):
    ...
    # print("f0_len: ", len(f0))
    # print("mel_len: ", len(mel))
    # print("pitch: ", pitch_coarse)
...
def build_phone_encoder(data_dir):
    phone_list_file = os.path.join(data_dir, 'phone_set.json')  # data_dir='data/binary/ljspeech'
    phone_list = json.load(open(phone_list_file))  #phone_list= ['!', ',', '.', ';', '<BOS>', '<EOS>', '?', 'AA0', 'AA1', 'AA2',
    return TokenTextEncoder(None, vocab_list=phone_list, replace_oov=',')  # 编码
...
def sinusoidal_smoothing_conv1d(input_signal, hparams):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    timestep = hparams['hop_size'] / hparams['audio_sample_rate']
    kernel_size = round(hparams['midi_smooth_width'] / timestep)
    padding = kernel_size // 2  # 手动计算“same”填充
    # 将输入信号转换为浮点型
    input_signal = input_signal

    # 对输入信号进行填充以处理“same”填充
    input_signal = F.pad(input_signal, (padding, padding - 1), mode='replicate')
    # 创建正弦平滑卷积核
    ...
    # 应用卷积
    print("++++++++++++++++++++++++++++++++++")
    smoothed_signal = F.conv1d(input_signal.to("cpu"), smooth_kernel[None, None].to("cpu"))
    print("----------------------------------")
